...
def extract_chapter_fully_fast(grade_name, folder_name, ch_num, base_dir, crops_dir):
    ...
    # 1. Question Pages: Render High-Res (DPI 150) and Fast OCR (DPI 72)
    q_pages = []
    for p_idx in range(len(q_doc)):
        ...
        pix_low = page.get_pixmap(dpi=72)
        img_low = Image.frombytes('RGB', [pix_low.width, pix_low.height], pix_low.samples)
        scale = pix_hi.height / pix_low.height

        res, _ = ocr(np.array(img_low))
        lines = []
        if res:
            for item in res:
                box, text, score = item
        ...
        q_pages.append({'img': img_hi, 'lines': lines, 'width': pix_hi.width, 'height': pix_hi.height})

    # Detect Chapter Title
    ch_title = f"第{ch_num}讲"
    for l in q_pages[0]['lines'][:8]:
        txt = l['text']
    ...
    for p_idx, p_data in enumerate(q_pages):
        for l_idx, l in enumerate(p_data['lines']):
            ...
            if m:
                ...
                if 1 <= q_num <= 25 and l['y0'] < p_data['height'] - 100:
                    q_anchors.append({
    ...
    # 2. Answer Pages: Fast OCR (DPI 72)
    ans_pages = []
    for ap_idx in range(len(ans_doc)):
        ...
        pix_low = apage.get_pixmap(dpi=72)
        img_low = Image.frombytes('RGB', [pix_low.width, pix_low.height], pix_low.samples)
        scale = pix_hi.height / pix_low.height
        ...
        if res:
            for item in res:
                ...
                x1 = max(p[0] for p in box) * scale
                lines.append({'y0': y0, 'y1': y1, 'x0': x0, 'x1': x1, 'text': text.strip()})
        ans_pages.append({'img': img_hi, 'lines': lines, 'width': pix_hi.width, 'height': pix_hi.height})
    ...
    for ap_idx, ap_data in enumerate(ans_pages):
        for l_idx, l in enumerate(ap_data['lines']):
            ...
            if m:
                q_num = int(m.group(1))
                if 1 <= q_num <= 25 and l['y0'] < ap_data['height'] - 100:
                    ans_anchors.append({
                        'section': cur_ans_sec,
                        'q_num': q_num,
                        'p_idx': ap_idx,
...
def main():
    ...
    print("Extracting 100% authentic questions and precise image crops across all 15 GaoSi chapters...")
    for grade_name, folder_name, chapter_nums in grade_configs:
        for ch_num in chapter_nums:
            ch_obj, qs = extract_chapter_fully_fast(grade_name, folder_name, ch_num, base_dir, crops_dir)
            if ch_obj:
